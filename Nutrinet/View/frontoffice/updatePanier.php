...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (
        ...
        error_log("All required POST fields are set.");
        if (
            !empty($_POST['produit']) &&
            !empty($_POST["prix"]) &&
            ...
            error_log("All required POST fields are not empty.");
            ...
            $panierC->updatePanier($panierItem,$_GET['idPanier']);

            error_log("Panier item added successfully.");

            header('Location: panier.php');
            exit(); // Make sure to exit after header redirect to prevent further execution
        } else {
            // Log an error message to the console
            error_log("All fields are required or empty.");
...
<!-- ... (your remaining HTML and script tags) ... -->


         <h3>modifier un produit</h3>
         <?php
            if (isset($_GET['idPanier'])) {
            $Panier = $panierC->showPanier($_GET['idPanier']);
         ?>
         <form method="POST" action="">
            <div class="form-group">
               <label for="produit">Produit:</label>
               <input type="text" class="form-control" id="produit" name="produit" value="<?php echo $Panier['produit']; ?>" required >
            </div>
            <div class="form-group">
               <label for="prix">Prix:</label>
               <input type="number" class="form-control" id="prix" name="prix" value="<?php echo $Panier['prix']; ?>" required >
            </div>
            <div class="form-group">
               <label for="quantite">Quantité:</label>
               <input type="number" class="form-control" id="quantite" name="quantite" value="<?php echo $Panier['quantite']; ?>" required >
            </div>
            <div class="form-group">
               <label for="idItems">idItems:</label>
               <input type="text" class="form-control" id="idItems" name="idItems" value="<?php echo $Panier['idItems']; ?>" required >
            </div>
            <div class="form-group">
               <label for="idUser">idUser:</label>
               <input type="text" class="form-control" id="idUser" name="idUser" value="<?php echo $Panier['idUser']; ?>" readonly>
            </div>
            <br>
            <button type="submit" class="btn btn-primary" style="margin-right: 73%">Modifier le panier</button>
         </form>
         <?php
                }
         ?>
